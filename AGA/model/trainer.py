...
class Trainer(object):
    ...
    def train_epoch(self, epoch, loss_weights):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            sep_loss = [torch.tensor(0.0, requires_grad=True) if torch.isnan(torch.tensor(l)) else torch.tensor(l, requires_grad=True) for l in sep_loss]
            ...
            total_sep_loss += np.array([l.item() for l in sep_loss])
    ...
    def val_epoch(self, epoch, val_dataloader, loss_weights):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                sep_loss = [torch.tensor(0.0) if torch.isnan(torch.tensor(l)) else torch.tensor(l) for l in sep_loss]
    ...
    def test(model, dataloader, scaler, graph, logger, args):
        ...
        #print("Shape of y_true before inverse transform:", y_true.shape)
        ...
        print("Shape of y_true after inverse transform:", y_true.shape)
        print("Shape of y_pred after inverse transform:", y_pred.shape)
        y_true = y_true.expand_as(y_pred)

        test_results = [] 

        mae, mape = test_metrics(y_pred, y_true)
        logger.info("MAE: {:.2f}, MAPE: {:.4f}%".format(mae, mape*100))
